...
//改变订单状态
    ...
    public function doPageOrderInfo()
    {
        ...
        $res['yj_money'] = number_format($res['money'] - ($storetype['poundage'] * ($res['money'] - $res['ps_money']) / 100 + ($res['ps_money'] * $store['ps_poundage']) / 100), 2);
        $data['order'] = $res;
        $data['good'] = $res2;
        $data['store'] = $res3;
        $data['storeset'] = $res4;
...
//模板消息
    public function doPageMessage()
    {
        ...
        $url = "https://api.weixin.qq.com/cgi-bin/message/subscribe/send?access_token=".$access_token."";
            ...
            $data = curl_exec($ch);
            curl_close($ch);
            $res=pdo_delete('cjdc_dingyue',array('user_id'=>$res2['user_id'],'tpl_id'=>$res["xd_tid"]));
            return $data;
        }
...
//充值模板消息
    public function doPageCzMessage()
    {
      ...
      $url = "https://api.weixin.qq.com/cgi-bin/message/subscribe/send?access_token=".$access_token."";
            ...
            curl_close($ch);
            // return $data;
            $res=pdo_delete('cjdc_dingyue',array('user_id'=>$user['id'],'tpl_id'=>$res["cz_tid"]));
        }
...
//入驻支付
    public function doPageRzPay()
    {
        ...
        $out_trade_no = $mch_id . time();
        $root = $_W['siteroot'];
        pdo_update('cjdc_store', array('code' => $out_trade_no), array('id' => $_GPC['rz_id']));
        $total_fee = $_GPC['money'];
        if (empty($total_fee)) //押金
        {
            $body = $res2['url_name'];
...
//入驻模板消息
    public function doPageRzMessage()
    {
        ...
        $url = "https://api.weixin.qq.com/cgi-bin/message/subscribe/send?access_token=".$access_token."";
            ...
            $data = curl_exec($ch);
            curl_close($ch);
            $res=pdo_delete('cjdc_dingyue',array('user_id'=>$res2['id'],'tpl_id'=>$res["rzsh_tid"]));
            return $data;
        }
...
$url = "https://api.weixin.qq.com/cgi-bin/message/subscribe/send?access_token=".$access_token."";
            ...
            $data = curl_exec($ch);
            curl_close($ch);
            $res=pdo_delete('cjdc_dingyue',array('user_id'=>$user['id'],'tpl_id'=>$res["jd_tid"]));
            // return $data;
        }
...
$url = "https://api.weixin.qq.com/cgi-bin/message/subscribe/send?access_token=".$access_token."";
                    ...
                    $data = curl_exec($ch);
                    curl_close($ch);
                    $res=pdo_delete('cjdc_dingyue',array('user_id'=>$user['id'],'tpl_id'=>$res["jj_tid"]));
                    // return $data;
    ...
    public function doPageTkTg()
    {
      ...
      $url = "https://api.weixin.qq.com/cgi-bin/message/subscribe/send?access_token=".$access_token."";
                ...
                curl_close($ch);
                // return $data;
                pdo_delete('cjdc_dingyue', array('user_id' => $user['id'],'tpl_id'=>$res["tk_tid"]));
            }
...
//新订单模板消息
    public function doPageNewOrderMessage()
    {
      ...
      $url = "https://api.weixin.qq.com/cgi-bin/message/subscribe/send?access_token=".$access_token."";
            ...
            curl_close($ch);
            // return $data;
           pdo_delete('cjdc_dingyue', array('user_id' => $user['id'],'tpl_id'=>$res["xdd_tid"]));
        }
...
//新订单模板消息
    public function doPageNewDmOrderMessage()
    {
      ...
      $url = "https://api.weixin.qq.com/cgi-bin/message/subscribe/send?access_token=".$access_token."";
            ...
            curl_close($ch);
            // return $data;
            pdo_delete('cjdc_dingyue', array('user_id' => $user['id'],'tpl_id'=>$res["xdd_tid2"]));
        }
...
//抢购支付
    public function doPageQgPay()
    {
        ...
        $out_trade_no = $mch_id . time();
        $root = $_W['siteroot'];
        pdo_update('cjdc_qgorder', array('code' => $out_trade_no), array('id' => $_GPC['order_id']));
        $total_fee = $_GPC['money'];
        if (empty($total_fee)) //押金
        {
            $body = $res2['url_name'];
...
//拼团支付
    public function doPageGroupPay()
    {
        ...
        $out_trade_no = $mch_id . time();
        $root = $_W['siteroot'];
        pdo_update('cjdc_grouporder', array('code' => $out_trade_no), array('id' => $_GPC['order_id']));
        $total_fee = $_GPC['money'];
        if (empty($total_fee)) //押金
        {
            $body = $res2['url_name'];
...
//取号成功模板消息
    public function doPageNumberMessage()
    {
      ...
      $url = "https://api.weixin.qq.com/cgi-bin/message/subscribe/send?access_token=".$access_token."";
            ...
            curl_close($ch);
            // return $data;
           pdo_delete('cjdc_dingyue', array('user_id' => $user['id'],'tpl_id'=>$res["qh_tid"]));
        }
...
//*********************配置项*************************
    ...
    public function doPageDlist(){
    	...
    	global $_GPC, $_W;
		$info = pdo_get('cjdc_message',array('uniacid'=>$_W['uniacid']));
        $model['xd_tid'] =$info['xd_tid'];
        $model['jd_tid'] = $info['jd_tid'];
        $model['jj_tid'] = $info['jj_tid'];
        $model['tk_tid'] =$info['tk_tid'];
        $model['yy_tid'] =$info['yy_tid'];
        $model['rzsh_tid'] =$info['rzsh_tid'];
        $model['rzcg_tid'] =$info['rzcg_tid'];
        $model['cz_tid'] =$info['cz_tid'];
        $model['xdd_tid'] =$info['xdd_tid'];
        $model['xdd_tid2'] =$info['xdd_tid2'];
        $model['qh_tid'] =$info['qh_tid'];


		$arr = ['xd_tid' => '外卖下单通知', 'jd_tid' => '外卖接单通知', 'jj_tid' => '拒绝接单通知', 'tk_tid' => '退款通知', 'yy_tid' => '预约通过通知', 'rzsh_tid' => '入驻申请通知', 'rzcg_tid' => '入驻审核通知', 'rzcg_tid' => '用户充值成功通知', 'xdd_tid' => '商家外卖新订单提醒通知', 'xdd_tid2' => '商家当面付新订单提醒通知', 'qh_tid' => '取号成功通知'];
		$new_list = [];
		...
		foreach ($new_list as $key => $value) {
            $new_list[$key]['rec'] = pdo_get('cjdc_dingyue',array('uniacid'=>$_W['uniacid'],'user_id'=>$_GPC['user_id'],'tpl_id'=>$value['tpl_id'],'tpl_name'=>$value['tpl_name']));
              if ($new_list[$key]['rec']) {
    ...
    public function doPageSubscribe(){ 
       ...
       // echo "<pre>";print_r($detail);die;
         $res=pdo_insert('cjdc_dingyue',$detail);
